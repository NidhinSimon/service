...
const Sidebarcoupon = ({ sidebar ,selectedCoupon,setSelectedcoupon,closeSidebar}) => {
...
toast.success(`${selectedCoupon.couponName},${selectedCoupon.discount}% off applied`)
          ...
          <div className="rounded-md w-full p-5 h-auto  mt-5 border border-zinc-300 " >
            <h1 className="font-semibold text-center">Available coupons</h1>
                ...
                {available.map((i)=>(
                   ...
                   <p className="font-semibold max-w-lg ml-2">Get {i.discount}% off on services </p>
                   <button onClick={()=>handlecouponselect(i._id)} className="border border-orange-400 h-10 w-32 rounded-lg text-orange-400 mt-5">
                     Apply Coupon
                   </button>
                   </div>
                   </>
